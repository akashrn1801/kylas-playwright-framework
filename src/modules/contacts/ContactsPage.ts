...
export class ContactsPage extends BasePage {
  ...
  private async waitForContactListPage(): Promise<void> {
    ...
    await this.waitForUrlWithNavigationRetry(`${config.appUrl}/sales/contacts/list`, /contacts\/list/);
  ...
  async cloneContact(): Promise<number | null> {
    ...
    if (await phoneInput.isVisible().catch(() => false)) {
      ...
      // WHY (2026-09-07, PROD Build #4, hardened based on review — root
      // cause not confirmed, per rule 10): a one-time PROD "Enter valid
      // phone number" clone failure (recovered on Playwright's own retry)
      // is generated by a faker expression that is structurally guaranteed
      // valid every call — the value itself cannot be malformed. The only
      // plausible mechanism left is the same class of async-commit race
      // already fixed once in this codebase for DealsPage.cloneDeal() (the
      // Save click landing before React had actually absorbed a just-filled
      // value). Confirm the field's committed value before proceeding,
      // matching that precedent, rather than trusting fill() alone.
      // WHY wrapped in withSessionExpiryRecovery(): standing rule 3 — any new
      // raw Playwright assertion in a module file, not already covered by an
      // existing BasePage helper, must be protected against a mid-wait
      // session expiry.
      await this.withSessionExpiryRecovery(() =>
